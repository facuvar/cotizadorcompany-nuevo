...
try {
    ...
    // Obtener productos (solo los que son de la categoría ASCENSORES)
    $query = "SELECT p.* FROM opciones p 
...
<body>
        ...
        <div class="step-content active" id="step-1">
            <div class="card">
                ...
                <div class="product-grid">
                    <?php 
                    // Mostrar productos de la categoría ASCENSORES
                    if ($productos && $productos->num_rows > 0):
                        while ($producto = $productos->fetch_assoc()):
                    ?>
                            <div class="product-item" data-id="<?php echo $producto['id']; ?>" data-precio="<?php echo $producto['precio']; ?>">
                                ...
                                <?php endif; ?>
                                <div class="option-price">$<?php echo number_format($producto['precio'], 2, ',', '.'); ?></div>
                            </div>
                    <?php 
                            endwhile; 
                        else: 
                    ?>
                        <p>No hay productos disponibles en la categoría ASCENSORES.</p>
                    <?php endif; ?>
                </div>
        ...
        <div class="step-content" id="step-2">
            <div class="card">
                ...
                <div class="plazo-selector" style="margin-bottom: 20px;">
                    ...
                    <select id="plazoSelectPaso2" style="margin-left: 10px; width: auto;">
                        <?php if ($plazos && $plazos->num_rows > 0): 
                            // Reiniciar el puntero del resultado
                            $plazos->data_seek(0);
                            while ($plazo = $plazos->fetch_assoc()): ?>
                                <option value="<?php echo $plazo['nombre']; ?>" 
                                    <?php echo ($plazo['nombre'] == $plazoSeleccionado) ? 'selected' : ''; ?>>
                                    <?php echo htmlspecialchars($plazo['nombre']); ?>
                                    <?php if (!empty($plazo['descripcion'])): ?> - <?php echo htmlspecialchars($plazo['descripcion']); ?><?php endif; ?>
                                </option>
                            <?php endwhile; ?>
                        <?php else: ?>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Productos que tienen adicionales
            const productosConAdicionales = [
                'ADICIONALES ASCENSORES ELECTROMECANICOS',
            ...
            productItems.forEach(item => {
                item.addEventListener('click', function() {
                    ...
                    const nombreProducto = this.querySelector('.product-title').textContent;
                    selectedProduct = {
                        id: this.dataset.id,
                        nombre: nombreProducto,
                        tieneAdicionales: productosConAdicionales.includes(nombreProducto)
                    };
                    
                    console.log('Producto tiene adicionales:', selectedProduct.tieneAdicionales);
            ...
            function cargarOpciones(productoId, plazoNombre = selectedPlazo) {
                optionsContainer.innerHTML = '<p>Cargando opciones...</p>';
                
                // Usar AJAX para cargar las opciones del producto seleccionado
                cargarOpcionesAJAX(productoId, plazoNombre);
            }
            
            // Función alternativa para cargar opciones (si necesitamos usar AJAX)
            function cargarOpcionesAJAX(productoId, plazoNombre = selectedPlazo) {
                optionsContainer.innerHTML = '<p>Cargando opciones...</p>';
                
                // Realizar petición AJAX para obtener opciones con el plazo seleccionado
                fetch(`get_opciones.php?producto_id=${productoId}&plazo_nombre=${encodeURIComponent(plazoNombre)}`)
                        ...
                        if (data.length > 0) {
                            ...
                            data.forEach(opcion => {
                                ...
                                optionItem.addEventListener('click', function() {
                                    console.log('Opción seleccionada:', this.dataset.id);
                        ...
                        if (data.length > 0) {
                            ...
                            data.forEach(adicional => {
                                ...
                                adicionalItem.addEventListener('click', function() {
                                    console.log('Adicional toggle:', this.dataset.id);
            ...
            plazoSelectPaso2.addEventListener('change', function() {
                ...
                console.log('Plazo seleccionado:', selectedPlazo);
            ...
            nextButtons.forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    // Si el producto tiene adicionales y vamos al paso de adicionales, cargarlos
                    if (nextStep === 3 && selectedProduct.tieneAdicionales) {
                        cargarAdicionales(selectedProduct.id);
                    }
                    
                    // Si vamos al paso de resumen, actualizarlo
                    // O si vamos al paso 3 pero el producto no tiene adicionales, saltar al resumen
                    if (nextStep === 4 || (nextStep === 3 && !selectedProduct.tieneAdicionales)) {
                        // Si el producto no tiene adicionales y vamos al paso 3, saltar al paso 4
                        if (nextStep === 3 && !selectedProduct.tieneAdicionales) {
                            nextStep = 4;
                        }
                        actualizarResumen();
            ...
            prevButtons.forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    console.log('Navegación: volviendo al paso', prevStep);
